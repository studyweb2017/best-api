...
export default class MemberCtrl extends BaseCtrl {
  ...
  getInfo(id: string) {
    return Observable.from(MemberModel.findOne({_id: mongoose.Types.ObjectId(id)}).select('id account name avatarUrl email weixin apiInform testInform').exec())
    .map((doc:any) => {
      console.log(doc)
      let u = doc._doc
      return {
  ...
  post(member: any) {
    ...
    member.avatarUrl = member.avatarUrl || this.createAvatar(member.account)
