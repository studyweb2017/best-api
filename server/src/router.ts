...
//   // 接口
//   .get('/project/:pid/api', (ctx: any) => handle(ctx, InterfaceCtrl.get(ctx.params.pid,ctx.user._id, ctx.user.isAdmin)))
//   .get('/project/:pid/api/module', (ctx: any) => handle(ctx, InterfaceCtrl.getModule(ctx.params.pid,ctx.user._id, ctx.user.isAdmin)))
//   .get('/project/:pid/api/:id', (ctx: any) => handle(ctx, InterfaceCtrl.getById(ctx.params.pid, ctx.params.id,ctx.user._id, ctx.user.isAdmin)))
//   .get('/project/:pid/api/:id/version', (ctx: any) => handle(ctx, InterfaceCtrl.getVersionById(ctx.params.pid, ctx.params.id,ctx.user._id, ctx.user.isAdmin)))
//   .post('/project/:pid/api', (ctx: any) => handle(ctx, InterfaceCtrl.post(ctx.params.pid, ctx.request.fields,ctx.user._id, ctx.user.isAdmin)))
//   .put('/project/:pid/api/:id', (ctx: any) => handle(ctx, InterfaceCtrl.put(ctx.params.pid, ctx.params.id, ctx.request.fields, ctx.user._id, ctx.user.isAdmin)))
//   .del('/project/:pid/api/:id', (ctx: any) => handle(ctx, InterfaceCtrl.delete(ctx.params.pid, ctx.params.id, ctx.user._id, ctx.user.isAdmin)))
