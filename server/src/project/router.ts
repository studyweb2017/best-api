...
export default projectRouter.router
  ...
  .post('/project', (ctx: any) => projectRouter.handle(ctx, projectCtrl.post(ctx.request.fields)))
  .put('/project/:id', (ctx: any) => projectRouter.handle(ctx, projectCtrl.put(ctx.params.id, ctx.request.fields, ctx.user._id, ctx.user.isAdmin)))
  .del('/project/:id', (ctx: any) => projectRouter.handle(ctx, projectCtrl.delete(ctx.params.id, ctx.user._id, ctx.user.isAdmin)))
