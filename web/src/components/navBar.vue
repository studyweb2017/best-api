  ...
  div.nav-bar.ta-l.p-r
    ...
    div.f-r
      ...
      span.d-ib.user(v-show='user.name', v-popover:popover1="")
        ...
        img.avatar(:src='user.avatarUrl', alt='avatar', :title='user.name')
...
export default class NavBar extends Vue {
  ...
  async refresh() {
    ...
    let systemInfo: any = Cache.get('systemInfo') || await http.get('/api/setting')
    this.company = systemInfo.companyName
    this.logo = systemInfo.companyLogo
    if (systemInfo) {
      Cache.set('systemInfo', {
        companyLogo: systemInfo.companyLogo,
        companyName: systemInfo.companyName
      })
    }
